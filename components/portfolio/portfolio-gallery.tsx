...
const ModelViewer = dynamic(() => import("@/components/portfolio/model-viewer"), {
  ssr: false,
  loading: () => (
...
export default function PortfolioGallery() {
  ...
  const selected = portfolioCases.find((c) => c.id === selectedId) ?? filtered[0];
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-muted-foreground">
            Filter by specialty, compare raw scans against final ExoCad designs, and explore
            interactive 3D previews directly in your browser.
          </p>
        </div>
            ...
            <div className="lg:col-span-3">
              <Card className="overflow-hidden">
                ...
                <CardContent className="p-4 sm:p-6">
                  <ModelViewer 
      src={selected.htmlUrl} 
      title={selected.title} 
    />
                  <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
                    <div className="rounded-lg bg-muted/50 p-3">
                      <div className="text-xs text-muted-foreground">Material</div>
                      <div className="mt-0.5 text-sm font-medium">{selected.material}</div>
                    </div>
                    <div className="rounded-lg bg-muted/50 p-3">
                      <div className="flex items-center gap-1 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" /> Turnaround
                      </div>
                      <div className="mt-0.5 text-sm font-medium">{selected.turnaround}</div>
                    </div>
